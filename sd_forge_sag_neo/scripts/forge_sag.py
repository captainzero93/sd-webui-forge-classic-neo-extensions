...
class SAGForForge(scripts.Script):
    ...
    def process_before_every_sampling(self, p, *script_args, **kwargs):
        ...
        try:
            ...
            # Enhanced Flux detection
            if is_flux_model(unet):
                gr.Info("Self Attention Guidance is not compatible with Flux models")
                print("Self Attention Guidance is not compatible with Flux models")
                return
            
            # Check if it's a legacy model (SD 1.x, 2.x, SDXL)
            if hasattr(shared.sd_model, 'is_webui_legacy_model'):
                if not shared.sd_model.is_webui_legacy_model():
                    gr.Info("Self Attention Guidance only supports SD 1.x, SD 2.x, and SDXL models")
                    print("Self Attention Guidance only supports SD 1.x, SD 2.x, and SDXL models")
                    return
        except Exception as e:
            print(f"Warning: Could not determine model compatibility for SAG: {e}")
        ...
        if p.is_hr_pass == True and p.hr_cfg <= 1:
            gr.Info("Self Attention Guidance (hires pass) requires Hires CFG > 1")
